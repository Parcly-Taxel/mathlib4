...
at most $n$ by $m < 2n$. So $m = 2n-1$; comparing coefficients gives $m = 5, n = 3$
...
lemma monic_numerpol : (X ^ m + X - C (1 : ℤ)).Monic := by
  apply Monic.sub_of_left; swap
  · rw [eq_intCast, Int.cast_one, degree_one, degree_add_eq_left_of_degree_lt]; swap
    · rw [degree_X_pow, degree_X, Nat.one_lt_cast]; lia
    simp_rw [degree_X_pow, Nat.cast_pos]; lia
  apply monic_X_pow_add; rw [degree_X, Nat.one_lt_cast]; lia
...
lemma monic_denompol : (X ^ n + X ^ 2 - C (1 : ℤ)).Monic := by
  apply Monic.sub_of_left; swap
  · rw [eq_intCast, Int.cast_one, degree_one, degree_add_eq_left_of_degree_lt]; swap
    · simp_rw [degree_X_pow, Nat.cast_lt]; lia
    simp_rw [degree_X_pow, Nat.cast_pos]; lia
  apply monic_X_pow_add; rw [degree_X_pow, Nat.cast_lt]; lia
...
lemma dvd_of_hyp (h : {a : ℤ | 0 < a ∧ a ^ n + a ^ 2 - 1 ∣ a ^ m + a - 1}.Infinite) :
  ...
  contrapose h; rw [Set.not_infinite]
  suffices ∃ Z : ℤ, ∀ a > 0, a ^ n + a ^ 2 - 1 ∣ a ^ m + a - 1 → a ≤ Z by
    obtain ⟨Z, hZ⟩ := this
    have f0Z : (Finset.Ioc 0 Z : Set ℤ).Finite := Finset.finite_toSet _
    refine f0Z.subset fun a ma ↦ ?_
    rw [Set.mem_setOf] at ma; rw [Finset.coe_Ioc, Set.mem_Ioc]; exact ⟨ma.1, hZ _ ma.1 ma.2⟩
  ...
  specialize tto 1 zero_lt_one; obtain ⟨Z₁, hZ₁⟩ := tto
  have enrG := eventually_no_roots _ mapGne0
  rw [Filter.eventually_atTop] at enrG; obtain ⟨Z₂, hZ₂⟩ := enrG; simp_rw [IsRoot.def] at hZ₂
  have enrR := eventually_no_roots _ mapRne0
  rw [Filter.eventually_atTop] at enrR; obtain ⟨Z₃, hZ₃⟩ := enrR; simp_rw [IsRoot.def] at hZ₃
  use ⌈max Z₁ (max Z₂ Z₃)⌉; intro a apos da; by_contra! ca
  rw [← Int.cast_lt (R := ℝ)] at ca; replace ca := (Int.le_ceil _).trans ca.le
  ...
  have key : (((a ^ m + a - 1) / (a ^ n + a ^ 2 - 1) : ℤ) : ℝ) =
    ...
    calc
      ...
      _ = (G * (F /ₘ G) + F %ₘ G).eval a / G.eval a := by
        rw [add_comm _ (F %ₘ G), modByMonic_add_div F monG]; simp [F, G]
      _ = _ := by
  ...
  rw [hZ₁, Int.cast_zero] at key; apply absurd key
  exact (div_ne_zero hZ₃ hZ₂).symm
...
lemma exists_root_denompol :
  ...
  specialize ivt zeromem; rwa [Set.mem_image] at ivt
...
lemma m_lt_twice_n (h : X ^ n + X ^ 2 - C (1 : ℤ) ∣ X ^ m + X - C 1) : m < 2 * n := by
  by_contra! hm
  obtain ⟨rb, rpoln⟩ := r₀_spec hn; have rpolm := r₀_spec' hn h; set r := r₀ hn
  ...
  have key : 1 - r ≤ (1 - r ^ 2) ^ 2 := by
    calc
      ...
      _ ≤ (r ^ n) ^ 2 := by rw [← pow_mul, mul_comm]; exact pow_le_pow_of_le_one rb.1.le rb.2.le hm
      _ = _ := by congr 1; linarith only [rpoln]
  replace key : 0 ≤ r * (r - 1) * (r ^ 2 + r - 1) := by linarith
  apply absurd key; rw [not_le]; apply mul_neg_of_neg_of_pos (by nlinarith only [rb])
  rw [← rpolm]; gcongr ?_ + _ - _; exact pow_lt_pow_right_of_lt_one₀ rb.1 rb.2 (by lia)
...
lemma n_lt_m (h : X ^ n + X ^ 2 - C (1 : ℤ) ∣ X ^ m + X - C 1) : n < m := by
  ...
  apply lt_of_le_of_ne nlem; by_contra h'; subst h'
  rw [← dvd_sub_self_left,
    show X ^ n + X ^ 2 - C (1 : ℤ) - (X ^ n + X - C 1) = X ^ 2 - X by ring] at h
  have subn0 : X ^ 2 - X ≠ (0 : Polynomial ℤ) := by
    rw [sq, ← sub_one_mul]; exact mul_ne_zero (X_sub_C_ne_zero 1) X_ne_zero
  have hn' := natDegree_le_of_dvd h subn0
  suffices (X ^ 2 - X : Polynomial ℤ).natDegree = 2 by rw [natDegree_denompol hn] at hn'; lia
  compute_degree!
...
lemma m_n_eq (c₁ : m < 2 * n) (c₂ : n < m)
  ...
  suffices X ^ n + X ^ 2 - C (1 : ℤ) = X ^ (m - n) * (X + 1) - C 1 by
    ...
    have c₃ : n = m - n + 1 := by
      ...
      rw [natDegree_add_eq_left_of_degree_lt] at deq; swap
      · simp_rw [degree_X_pow, Nat.cast_lt]; lia
      rw [natDegree_add_eq_left_of_degree_lt] at deq; swap
      · simp_rw [degree_X_pow, Nat.cast_lt]; lia
    ...
    have deq := congr(natDegree $this)
    simp_rw [natDegree_X_pow] at deq; lia
  suffices X ^ n + X ^ 2 - C (1 : ℤ) ∣ X ^ (m - n) * (X + C 1) - C 1 by
    symm; apply eq_of_monic_of_dvd_of_natDegree_le (monic_denompol hn) _ this
    · rw [natDegree_sub_eq_left_of_natDegree_lt]; swap
      · rw [natDegree_C, natDegree_mul (by simp) (X_add_C_ne_zero 1), natDegree_X_pow,
          natDegree_X_add_C]; lia
      rw [natDegree_mul (by simp) (X_add_C_ne_zero 1), natDegree_X_pow, natDegree_X_add_C,
        natDegree_sub_eq_left_of_natDegree_lt]; swap
      · rw [natDegree_C, natDegree_add_eq_left_of_natDegree_lt]; swap
        · simp_rw [natDegree_X_pow]; lia
        rw [natDegree_X_pow]; lia
      rw [natDegree_add_eq_left_of_natDegree_lt]; swap
      · simp_rw [natDegree_X_pow]; lia
      rw [natDegree_X_pow]; lia
    · apply Monic.sub_of_left
      · exact (monic_X_pow _).mul (monic_X_add_C _)
      · rw [degree_C one_ne_zero, degree_mul, degree_X_pow, degree_X_add_C]; norm_cast; lia
  suffices X ^ n + X ^ 2 - C (1 : ℤ) ∣ X ^ n * (X ^ (m - n) * (X + C 1) - C 1) by
    ...
    apply IsRelPrime.mul_add_right_left; apply IsRelPrime.neg_left
    rw [C_1]; exact isRelPrime_one_left
...
theorem result : {a : ℤ | 0 < a ∧ a ^ n + a ^ 2 - 1 ∣ a ^ m + a - 1}.Infinite ↔ m = 5 ∧ n = 3 := by
  ...
  · rw [h.1, h.2]
    ...
    simp only [dvd_mul_left, and_true]; exact Set.Ioi_infinite _
